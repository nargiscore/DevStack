...
export default function Navbar() {
          ...
          {LINKS.map((link) => (
            <li key={link}>
              <a
                ...
                className="text-sm text-body transition-colors hover:text-[#DB2777]"
              >
                {link}
              </a>
            </li>
  ...
  <button className="hidden text-sm font-medium text-body hover:text-[#DB2777] sm:inline-block">
    Sign In
  </button>

  <button className="hidden rounded-full bg-[#D91B7E] px-4 py-2 text-sm font-semibold text-white transition-transform hover:scale-[1.03] sm:inline-flex">
    Sign Up
  </button>

  <button className="rounded-full bg-[#D91B7E] px-3.5 py-1.5 text-xs font-semibold text-white sm:hidden">
    Sign Up
  </button>
</div>
      </nav>
